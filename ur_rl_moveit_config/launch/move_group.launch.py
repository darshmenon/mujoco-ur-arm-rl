...
def generate_launch_description():
    ...
    # Package names
    package_name_ur_rl_moveit_config = 'ur_rl_moveit_config'
    ...
    declare_rviz_config_package_cmd = DeclareLaunchArgument(
        ...
        default_value=package_name_ur_rl_moveit_config,
    ...
    def configure_setup(context):
        ...
        ur_rl_moveit_config_share = FindPackageShare(package=package_name_ur_rl_moveit_config).find(package_name_ur_rl_moveit_config)

        # File paths from MoveIt config package
        urdf_path_xacro = os.path.join(ur_rl_moveit_config_share, "config", "ur.urdf.xacro")
        srdf_file_path = os.path.join(ur_rl_moveit_config_share, "config", f"{robot_name_str}.srdf")
        joint_limits_file_path = os.path.join(ur_rl_moveit_config_share, "config", "joint_limits.yaml")
        kinematics_file_path = os.path.join(ur_rl_moveit_config_share, "config", "kinematics.yaml")
        pilz_cartesian_limits_file_path = os.path.join(ur_rl_moveit_config_share, "config", "pilz_cartesian_limits.yaml")
        moveit_controllers_file_path = os.path.join(ur_rl_moveit_config_share, "config", "moveit_controllers.yaml")
        initial_positions_file_path = os.path.join(ur_rl_moveit_config_share, "config", "initial_positions.yaml")

        # Create MoveIt configuration
        ur_rl_moveit_config = (
            MoveItConfigsBuilder(robot_name_str, package_name=package_name_ur_rl_moveit_config)
        ...
        start_rviz_node_cmd = Node(
            ...
            output="screen",
            parameters=[
                ur_rl_moveit_config.robot_description,
                ur_rl_moveit_config.robot_description_semantic,
                ur_rl_moveit_config.planning_pipelines,
                ur_rl_moveit_config.robot_description_kinematics,
                ur_rl_moveit_config.joint_limits,
                {'use_sim_time': use_sim_time}
            ],
        )
